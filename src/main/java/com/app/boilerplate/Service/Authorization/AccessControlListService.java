...
public class AccessControlListService extends JdbcMutableAclService {
	...
	public void removePermission(Class<?> type, Long objectId, Permission permission, String sidName,
								 boolean principal) {
		performAclOperation(type, objectId, acl -> {
			final var sid = principal ? new PrincipalSid(sidName) : new GrantedAuthoritySid(sidName);
	...
	public void togglePermission(Class<?> type, Long objectId, Permission permission, String sidName,
								 boolean principal) {
		performAclOperation(type, objectId, acl -> {
			final var sid = principal ? new PrincipalSid(sidName) : new GrantedAuthoritySid(sidName);
			for (int i = 0; i < acl.getEntries()
	...
	public MutableAcl createAcl(ObjectIdentity objectIdentity, String sidName,
								boolean principal) {
